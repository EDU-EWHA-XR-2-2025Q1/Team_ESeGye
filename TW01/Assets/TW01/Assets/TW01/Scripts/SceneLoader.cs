...
public class SceneLoader : MonoBehaviour
{
    ...
    public void LoadNKDYScene(){

        Debug.Log("버튼 눌림 - 씬 이동 시도");

        if (Application.CanStreamedLevelBeLoaded("TW01_NKDY_Scene"))
        {
            Debug.Log("씬 전환 시작!");
            SceneManager.LoadScene("TW01_NKDY_Scene");
        }
        else
        {
            Debug.LogError("TW01_NKDY_Scene 씬이 Build Settings에 등록되지 않았거나 이름이 틀렸습니다!");
        }
    }
    public void LoadSJHScene(){

        Debug.Log("버튼 눌림 - 씬 이동 시도");

        if (Application.CanStreamedLevelBeLoaded("TW01_SJH_Scene"))
        {
            Debug.Log("씬 전환 시작!");
            SceneManager.LoadScene("TW01_SJH_Scene");
        }
        else
        {
            Debug.LogError("TW01_SJH_Scene 씬이 Build Settings에 등록되지 않았거나 이름이 틀렸습니다!");
        }
    }
}
